...
async function test() {
  try {
    ...
    const res = await axios.get("https://api-ta.neverland.my.id/api/v1/posts?page=1");
    console.log("No token:", res.data.data.data.length, "posts");

    // Let's login first to get a token
    console.log("Logging in...");
    const loginRes = await axios.post("https://api-ta.neverland.my.id/api/v1/auth/login", {
    ...
    if (token) {
      ...
      const resWithToken = await axios.get("https://api-ta.neverland.my.id/api/v1/posts?page=1", {
        headers: {
