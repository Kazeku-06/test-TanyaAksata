...
export default function UserProfileView({
  ...
  if (isUserError || !user) {
    return (
      <div className="px-6 py-8 text-center">
        ...
        <Link href="/users" className="text-sm text-[#0074cc] hover:underline">
  ...
  return (
    <div className="px-6 py-4">
      ...
      {/* ── Profile Header ── */}
      <div className="flex flex-col sm:flex-row gap-5 mb-6 pb-6 border-b border-[#e3e6eb]">
        <Avatar name={user.name} avatar={user.avatar} size="xl" />

        <div className="flex-1 min-w-0">
          {/* Name + follow button */}
          <div className="flex flex-wrap items-start gap-3 mb-1.5">
            <h1 className="text-2xl font-bold text-[#232629]">{user.name}</h1>
            {/* Badge level reputasi */}
            <span className={cn(
              "px-2 py-0.5 rounded text-xs font-semibold mt-1",
              level === "Expert" ? "bg-[#fdf3d0] text-[#a56600]" :
              level === "Pro"    ? "bg-[#e1ecf4] text-[#39739d]" :
              level === "Regular"? "bg-[#d4edda] text-[#2e6d44]" :
          ...
          {/* Bio */}
          {user.bio && (
            <p className="text-sm text-[#3b4045] mb-2">{user.bio}</p>
          )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        <StatCard
          icon={<span className="text-[#f48024] font-bold text-base">{user.reputation}</span>}
          label="Reputasi"
        />
        <StatCard
          icon={<FileText className="w-5 h-5 text-[#0a95ff]" />}
          label="Pertanyaan"
          value={user.posts_count ?? 0}
        />
        <StatCard
          icon={<Users className="w-5 h-5 text-[#2e6d44]" />}
          label="Pengikut"
          value={user.followers_count ?? 0}
        />
        <StatCard
          icon={<Users className="w-5 h-5 text-[#6a737c]" />}
          label="Mengikuti"
      ...
      {user.badges && user.badges.length > 0 && (
        <div className="mb-6">
          <h2 className="text-sm font-semibold text-[#232629] mb-2">Badge</h2>
          <div className="flex flex-wrap gap-2">
            {user.badges.map((badge) => (
              <span
                key={badge.id}
                title={badge.description}
                className="flex items-center gap-1.5 px-2.5 py-1 text-xs rounded border border-[#e3e6eb] bg-white text-[#232629] hover:border-[#babfc4] transition-colors"
              >
      ...
      {/* ── Posts ── */}
      <div>
        <h2 className="text-base font-semibold text-[#232629] mb-3">
          Pertanyaan oleh {user.name}
          {postsTotal > 0 && (
            <span className="ml-2 text-sm font-normal text-[#6a737c]">
              ({postsTotal})
            </span>
          )}
        </h2>
        ...
        {isLoadingPosts ? (
          ...
          <p className="text-sm text-[#6a737c]">Belum ada pertanyaan.</p>
        ) : (
          <>
...
function StatCard({
  ...
  return (
    <div className="flex items-center gap-3 p-3 border border-[#e3e6eb] rounded bg-[#f8f9f9]">
      <div className="flex-shrink-0">{icon}</div>
      <div>
        {value !== undefined && (
          <p className="font-semibold text-[#232629] text-sm">{value}</p>
        )}
        <p className="text-xs text-[#6a737c]">{label}</p>
      </div>
    </div>
  );
}
