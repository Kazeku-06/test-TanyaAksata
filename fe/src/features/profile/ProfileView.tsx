...
export default function ProfileView({
  ...
  if (!user) {
    return (
      <div className="px-6 py-8 text-center text-sm text-[#6a737c]">
        Silakan <Link href="/login" className="text-[#0074cc] hover:underline">masuk</Link> untuk melihat profil.
      </div>
    );
  }
  ...
  return (
    <div className="px-6 py-4">
          ...
          <form onSubmit={onSubmit} className="flex flex-col gap-4" noValidate>
            ...
            <div className="flex flex-col gap-1">
              ...
              <textarea
                ...
                {...register("bio")}
              />
              {errors.bio && <p className="text-xs text-[#c91d2e]">{errors.bio.message}</p>}
            </div>

            <Input label="Lokasi" placeholder="Jakarta, Indonesia" error={errors.location?.message} {...register("location")} />
        ...
        <div>
          {isLoadingBadges ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
              {badges.map((badge) => (
                <div key={badge.id} className="flex items-center gap-3 p-3 border border-[#e3e6eb] rounded bg-white">
      ...
      {/* Tab: Mengikuti */}
      {activeTab === "following" && (
        <UserList users={following} isLoading={isLoadingFollowing} emptyText="Belum mengikuti siapa pun." />
      )}

      {/* Tab: Pengikut */}
      {activeTab === "followers" && (
        <UserList users={followers} isLoading={isLoadingFollowers} emptyText="Belum ada pengikut." />
      )}
...
// Komponen kecil reusable untuk list user
function UserList({ users, isLoading, emptyText }: { users: User[]; isLoading: boolean; emptyText: string }) {
